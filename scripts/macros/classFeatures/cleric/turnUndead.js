...
async function targets({speaker, actor, token, character, item, args, scope, workflow}) {
    ...
    for (let i of Array.from(workflow.targets)) {
        if (helpers.raceOrType(i.actor) != 'undead') continue;
        if (i.actor.system.attributes.hp.value === 0) continue;
        if (i.actor.flags['5e-content']?.feature?.turnResistance) await helpers.createEffect(i.actor, effectData);
        if (i.actor.flags['5e-content']?.feature?.turnImmunity) await helpers.createEffect(i.actor, effectData2);
        validTargets.push(i.id);
    }
    helpers.updateTargets(validTargets);
}

async function saves({speaker, actor, token, character, item, args, scope, workflow}) {
    if (workflow.failedSaves.size === 0) return;
    let classIdentifier = helpers.getConfiguration(workflow.item, 'identifier') ?? 'cleric';
    ...
    await helpers.applyDamage(destroyTokens, '10000', 'none');
}
