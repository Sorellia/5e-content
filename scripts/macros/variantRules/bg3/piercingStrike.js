...
async function turn(effect) {
    let turn = effect.flags['5e-content']?.feature?.piercingStrike ?? 0;
    if (turn >= 1) {
        await helpers.removeEffect(effect);
        return;
    } 
    let updates = {
        'flags.5e-content.feature.piercingStrike': turn + 1
    };
...
async function strike({speaker, actor, token, character, item, args, scope, workflow}) {
    ...
    let validTypes = ['rapier','shortsword','trident','pike','dagger'];
    if (!validTypes.includes(workflow.item.system.baseItem)) return;
    let feature = helpers.getFeature(workflow.actor, 'Piercing Strike');
    if (!feature) return;
    if (!feature.system.uses.value) return;
    let queueSetup = await queue.setup(workflow.item.uuid, 'piercingstrike', 250);
    if (!queueSetup) return;
    let selection = await helpers.dialog(feature.name, constants.yesNo, 'Use Piercing Strike?');
    if (!selection) {
        queue.remove(workflow.item.uuid);
        return;
    }
    ...
    let effect = helpers.findEffect(workflow.targets.first().actor, 'Gaping Wounds');
...
async function shot({speaker, actor, token, character, item, args, scope, workflow}) {
    ...
    let validTypes = ['lightcrossbow','handcrossbow','heavycrossbow'];
    if (!validTypes.includes(workflow.item.system.baseItem)) return;
    let feature = helpers.getFeature(workflow.actor, 'Piercing Shot');
    if (!feature) return;
    if (!feature.system.uses.value) return;
    let queueSetup = await queue.setup(workflow.item.uuid, 'piercingShot', 250);
    if (!queueSetup) return;
    let selection = await helpers.dialog(feature.name, constants.yesNo, 'Use Piercing Shot?');
    if (!selection) {
        queue.remove(workflow.item.uuid);
        return;
    }
