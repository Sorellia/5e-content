...
export async function cleave({speaker, actor, token, character, item, args, scope, workflow}) {
    let validTypes = ['battleaxe', 'greataxe', 'halberd', 'greatsword'];
    ...
    if (!validWeapons.length) {
        ui.notifications.info('No valid weapons equipped!');
        return;
    }

    let weapon;
    if (validWeapons.length === 1) weapon = validWeapons[0];
    if (!weapon) [weapon] = await helpers.selectDocument('What Weapon?', validWeapons);
    if (!weapon) return;

    let nearbyFoes = helpers.findNearby(workflow.token, weapon.system.range.value, 'enemy', true);
    ...
    if (nearbyFoes.length > 3) {
        let selectedTargets = await helpers.selectTarget('Select your targets (Maximum: 3)', [{'label': 'Ok', 'value': true}, {'label': 'Cancel', 'value': false}], nearbyFoes, true, 'multiple');
