...
async function turn(effect) {
    let turn = effect.flags['5e-content']?.feature?.heartstopper ?? 0;
    if (turn >= 1) {
        await helpers.removeEffect(effect);
        return;
    } 
    let updates = {
        'flags.5e-content.feature.heartstopper': turn + 1
    };
...
async function attack({speaker, actor, token, character, item, args, scope, workflow}) {
    ...
    let validTypes = ['morningstar'];
    if (!validTypes.includes(workflow.item.system.baseItem)) return;
    let feature = helpers.getFeature(workflow.actor, 'Heartstopper');
    if (!feature) return;
    if (!feature.system.uses.value) return;
    let queueSetup = await queue.setup(workflow.item.uuid, 'heartstopper', 250);
    if (!queueSetup) return;
    let selection = await helpers.dialog(feature.name, constants.yesNo, 'Use Heartstopper?');
    if (!selection) {
        queue.remove(workflow.item.uuid);
        return;
    }
